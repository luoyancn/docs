...
同样的，函数也可以作为宏的参数进行使用：
\begin{code-block}{rust}
// 如果下面的*修改为+，则表示wait这个宏至少需要1个参数
#[macro_export]
macro_rules! wait {
    // ident表示变量名或函数名，path则表示路径，类似std::fmt
    //($($function_no_args: ident),*) => {{
...
fn main() {
    wait!(hello,bye);
    // 如果使用ident，下面的代码会报错
    // no rules expected the token `::`: when calling this macro
    // 因此实际使用中，最好使用path替代ident：
    wait!(datastructure::vec_usage);
}
\end{code-block}
    ...
    /*
     hashmap宏的内部规则, 等价于如下的一个宏，作用是返回接收到的元素的个数
       macro_rules! count {
           ...
           // let lenth = <[String]>::len(&["string".to_string(),
           //       "string".to_string()])
           // 性能更好，因为()不占据任何内存空间
           // let lenth = <[()]>::len(&[(), ()])
       ...
       如下：
       hashmap!(@unit $key), hashmap!(@count $($rest),*)
    */

    (@count $($rest:expr), *) => (<[()]>::len(&[$(hashmap!(@unit $rest)),*]));

    /*
     $($key:expr => $value:expr),* 表达式
     本身可以匹配hashmap!()，hashmap!("1"=>2)
...
如果在一个宏当中使用了其他的宏，但是这个被依赖的宏并不是当前的lib包的成员，此时
的export过程需要变化一下，将依赖宏当作函数进行导入，但是，需要使用pub关键字进行
标记，然后再在当前的lib当中进行导出：
\begin{code-block}{rust}
// 将其他lib库当中的宏以pub的方式进行导入
// as关键字进行重命名，避免和当前lib包的宏发生冲突
pub use slog_scope::{
...
// $crate表示当前的crate包，将当前包当中使用的函数或者方法进行重新导出
#[macro_export]
...
\subsection{语法树}
编写真正可用的过程宏比较复杂，但不管如何变化，Rust的宏都是依赖于
...
proc-macro = true
\end{code-block}
另外，当lib当中设置\codeinline{toml}{proc-macro=true}之后，
...
不可用，因此通常只能通过标准错误输出进行信息的打印。通常情况下，都是使用\codeinline{rust}{eprint!}
或者\codeinline{rust}{eprintln!}进行过程宏的调试输出。
...
来检测代码是否存在问题：\codeinline{bash}{cargo check}。
...
但是，需要注意expand指令只能在源码（即rs文件）所在路径执行：\codeinline{bash}{cargo expand}。
然后会得到类似如下的输出：
\begin{figure}[H]
  \centering
...
而这个错误，则是由\codeinline{rust}{parse_macro_input!}这个
宏提示出来的。
...
\end{code-block}
即利用\codeinline{rust}{#[derive(Builder)]}宏
对任意结构体实现工厂模式代码的自动生成。
...
\end{code-block}
注意，上述代码当中的st（语法树）结构是一个\codeinline{rust}{syn::DeriveInput}结构体\footnote{参考：\url{https://docs.rs/syn/1.0.72/syn/struct.DeriveInput.html}}，其内在结构如下：
\begin{code-block}{rust}
...
到此时，一个Builder属性宏的基本框架已经有了，通过\codeinline{bash}{cargo expand}将
...
\end{code-block}
由于本例当中，关注的是结构体，因此，需要重点关注\codeinline{rust}{DataStruct}
...
\end{code-block}
结构体的字段变量，放在了\codeinline{rust}{Fields}当中，
...
\end{code-block}
将有名字段的结构体继续进行展开，最终，我们会得到一个名为\codeinline{rust}{Field}
...
最后，将上述的2个函数结合到\codeinline{rust}{do_expand}函数当中，改造之后的函数则如下：
\begin{code-block}{rust}
fn do_expand(st: &syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
...
继续改造之后比较完整的\codeinline{rust}{do_expand}函数如下：
\begin{code-block}{rust}
fn do_expand(st: &syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
...
\end{code-block}
上述代码通过\codeinline{bash}{cargo expand}之后，
其结果大致如下：
\begin{figure}[H]
  \centering
