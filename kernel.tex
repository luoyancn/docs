...
{
        int ret = 0;
        char kernel_data[] = "hello read char dev function";
        size_t data_len = sizeof(kernel_data) / sizeof(char);
        if (count > data_len){
                count = data_len;
        }
        if ((ret = copy_to_user(user_buffer, kernel_data, count))){
                ...
                const char __user * user_buffer, size_t count, loff_t *ppos)
{
        ...
        if ((ret = copy_from_user(writebuf, user_buffer, count))){
                printk(KERN_ERR "Failed write to char device\n");
                return -EFAULT;
        }
        printk(KERN_INFO "Write to the starter char dev with content %s\n",
                writebuf);
        return 0;
}
\end{code-block}
...
static char writebuf[WRITE_LEN];
...
struct chrdev {
        ...
        struct cdev cdev;      // cdev结构体，用于注册字符设备
};
...
static int __init chrdev_init(void)
{
        ...
        result = alloc_chrdev_region(&char_device.dev_id, 0, 1, DEV_NAME);
        if (result) {
                printk(KERN_ERR
                        "Faild to allock char device number for"
