...
如果操作了Verified label，有可能会导致代码无法合并，解决方式如下：
\begin{code-block}{bash}
git clone "ssh://zhangjl@172.16.1.180:29418/All-Projects" \
    && (cd "All-Projects" && mkdir -p `git rev-parse --git-dir`/hooks/ \
    && curl -Lo `git rev-parse --git-dir`/hooks/commit-msg http://172.16.1.180:8080/tools/hooks/commit-msg \
    && chmod +x `git rev-parse --git-dir`/hooks/commit-msg)
sed -i '/Verified/,$d' project.config
git add project.config
git commit -m 'del verified'
