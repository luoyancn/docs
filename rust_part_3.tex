...
在此之前，我们看到的Trait全部作用于结构体上，但实际上，Trait也可以作用于函数以及
闭包上，因为Fn/FnMut/FnOnce本身也是特殊的类型。如下示例：
\begin{code-block}[escapeinside=~~, mathescape]{rust}
trait Executor { ~\namedlabel{define}{\ding{202}}~
...
impl<F> Executor for F ~\namedlabel{function}{\ding{204}}~
where
    F: Fn(&str) -> Result<String, &'static str>,
{
    fn execute(~\colorbox{WildStrawberry}{&self}\namedlabel{self}{\ding{205}}~, cmd: &str) -> Result<String, &'static str> {
        self(cmd)
    }
}
...
\begin{itemize}
  ...
  \item \coloref{define} 是常见的Trait的定义方式
  \item \coloref{struct} 是常见的Trait的实现方式，作用在struct上
  \item \colorunderline{\coloref{function}} 为函数与闭包实现Trait
  \item \colorunderline{\coloref{self}} self指向Fn/FnMut/FnOnce所指代的函数与闭包
\end{itemize}
另外需要说明的是，Fn/FnMut/FnOnce是被包含的关系，即前者被后者所包含。

\subsection{高级类型与系统Trait}
...
impl From<(i32, i32)> for Point { ~\namedlabel{fromtrait}{{\rightpointleft}From}~
...
impl TryFrom<(u8, u8)> for Point { ~\namedlabel{tryfromtrait}{{\rightpointleft}TryFrom}~
    type Error = ();
...
u64类型的，但由于虚类型不同，因此，\codeinline{rust}{assert_eq(user.id,product.id)}
无法编译通过。
...
impl Iterator for Fibonacci { ~\namedlabel{itertrait}{{\rightpointleft}Iterator}~
    type Item = u64;
...
除了上面的所介绍的\colorunderline{\coloref{fromtrait}}、\colorunderline{\coloref{tryfromtrait}}、\colorunderline{\coloref{itertrait}}以及Display等之外，rust当中常用并且应当熟悉的还有如下的几个：
\begin{itemize}
  \renewcommand\labelitemi{\leftpointright}
  \item 标记型
    \begin{itemize}
  ...
  \item \&[u8]实现了Read这个Trait，可直接生成对应的Reader
    \begin{itemize}
      \renewcommand\labelitemii{\ding{79}}
      \item \codeinline{rust}{let reader: &mut dyn Read = &mut message.as_bytes();}
    \end{itemize}
\end{itemize}
