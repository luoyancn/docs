...
\codeinline{rust}{impl<L, R, O> Adder<Private<R>> for Private<L> where L: Adder<R, Output = O>}。
在这里，我们将其展开进行讲解：
\begin{itemize}
  \item \codeinline{rust}{Private<L>}表示左操作数
  \item L、R都是泛型T的别名，O表示Trait Adder的关联数据类型，在本例当中，O指向了\codeinline{rust}{Private<T>}
  \item \codeinline{rust}{L: Adder<R, Output = O>,}表示L是Adder的一种具体实现，其类型与R相同，对应的关联类型为O，如果没有
