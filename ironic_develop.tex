...
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.3]{pecan.png}
  \caption{项目文件结构图}
  \label{fig:pecan}
\end{figure}
...
使用浏览器访问发布的服务，如图 \nameref{fig:quickstart}所示
\begin{figure}[H]
  \centering
  \includegraphics[scale=0.4]{quickstart.png}
  \caption{返回结果}
...
与\nameref{chap:start}章节不一样的是，在使用RestController的时候，Pecan提供了默认的路由信息。具体的
路由对照如下表
\begin{center}
