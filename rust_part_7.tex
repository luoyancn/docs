...
如果Future的返回结果如同\codeinline{rust}{pub fn stats() -> impl Future<Output = Result<CpuStats, Error>>}，
则使用方式大致如下：
\begin{code-block}{rust}
...
而如果Future的返回结果如\codeinline{rust}{pub fn partitions() -> impl Stream<Item = Result<Partition, Error>>}，
则使用方式大致如下：
\begin{code-block}{rust}
...
\codeinline{rust}{block_on}是进入Runtime的主要方式，但并不是唯一方式：还可以通过
\codeinline{rust}{enter}进入Runtime。但是，2者有很大的不同：\codeinline{rust}{block_on}
会阻塞当前的主线程，直到任务结束，而\codeinline{rust}{enter}只是陷入到Runtime当中，
并不会阻塞当前的主线程执行，它会返回一个\codeinline{rust}{EnterGuard}实例，该实例
...
Runtime不仅可以用于常见的模式，也可以用于tokio的注解方式。默认情况下，\codeinline{rust}{#[tokio::main]}
...
Runtime实现了\codeinline{rust}{Send}和\codeinline{rust}{Sync}这两个Trait，因此可
以将runtime包在\codeinline{rust}{Arc}里，然后跨线程使用同一个runtime。很明显的是，
...
\colorblock{单线程runtime对单个线程(单核CPU)的利用率，是高于多线程runtime的}。
...
Worker thread只用于执行那些异步任务，异步任务指的是不会阻塞线程的任务；blocking thread
则用于那些长时间计算的或阻塞整个线程的任务，只有在调用了\codeinline{rust}{spawn_blocking()}才会
创建一个对应的blocking thread。Blocking thread不用于执行异步任务，因此runtime不会
去调度管理这类线程，它们在本质上相当于一个\codeinline{rust}{thread::spawn()}创建的独立线程，
它也不会像\codeinline{rust}{block_on()}操作一样会阻塞当前线程。它和独立线程的唯
...
控制。因此，如果在\codeinline{rust}{block_on()}中生成了blocking thread或普通的线程，
\codeinline{rust}{block_on()}不会等待这些线程的完成，而是立即返回：
\begin{code-block}{rust}
rt.block_on(async{
...
当不再需要runtime时，可以通过drop操作，关闭runtime。一个runtime的完整关闭过程如下：
\begin{enumerate}
...
\codeinline{rust}{shutdown_timeout()}和\codeinline{rust}{shutdown_background()}，
...
但是需要注意，如果在无法移动的.await上使用Mutex，则非常容易导致死锁。通常的做法是
封装成\codeinline{rust}{Arc<Mutex<...>>}的模式。
\begin{code-block}{rust}
use std::sync::Arc;
use tokio::{self, sync};
...
Linux当中的信号量非常相似，可以认为是一个从0个执行权开始的信号量。其中，\codeinline{rust}{notified().await}
通常表示通知到达。如果当前没有执行权，则可以通过\codeinline{rust}{notify_one}
进行设置。值得注意的是，如果\codeinline{rust}{notify_one}在\codeinline{rust}{notified().await}
之前被调用，下一次的\codeinline{rust}{notified().await}会立即完成，并且消耗执行权；任何
后续的\codeinline{rust}{notified().await}调用，都需要等待新的执行权到达。如果在执行
\codeinline{rust}{notified().await}之前，\codeinline{rust}{notify_one}被调用了多次，
那只有一个执行权会被存储。
...
默认情况下，Notify都只会使用\codeinline{rust}{notify_one}通知一个监听者，但是，实际上
它也可以通知多个/所有的监听者：
\begin{code-block}{rust}
...
但是需要注意，该函数不会释放执行权，它会一次性唤醒所有正在等待的等候者。严格来说，
是让当前已经注册的等候者（即已经调用\codeinline{rust}{notified()}，但是还未执行await操作）
在下次等待的时候，可以直接通过。
\begin{critical}
如果等待者已经执行了\codeinline{rust}{notified().await}
操作，再执行该函数，则\colorblock{会导致死锁的出现}，比如下面的代码：
\begin{code-block}{rust}
...
\end{code-block}
但是，实际上，上述代码当中，如果我们将\codeinline{rust}{notify.notify_waiters()}
取消掉，代码仍然可以正常运行，但\colorblock{这是由于\codeinline{rust}{notified}的操作
没有执行\codeinline{rust}{await}操作造成的}，虽然看上去结果正确，但实际上，如果取消
\codeinline{rust}{notify_waiters}操作，从逻辑和原理上说，是错误的，notify的机制
根本就没有发挥任何作用。

\subsubsection{读写锁-RwLock与信号量-Semaphore}
...
同样的道理，也可以针对\codeinline{rust}{sleep}和\codeinline{rust}{timeout}设置
启动时间：
\begin{code-block}{rust}
...
而在实际的使用当中，定时器通常与\codeinline{rust}{tokio::select!}一起使用，关于
这个宏的说明以及具体的使用示例，将在后面进行展开。

和原始的Future-rs相同，tokio也支持同时运行多个task，常见的方法就是使用\codeinline{rust}{tokio::join!}
将多个task合并起来执行，比如：
\begin{code-block}{rust}
async fn __in__() {
    println!(123);
...
\end{code-block}
但是，需要注意，默认情况下，tokio的\codeinline{rust}{join!}是\colorblock{串行执行}的，也就是说，
如果其中第一个task被阻塞，后续的task都无法被执行。如果想让\codeinline{rust}{join!}的
任务也变成并行/并发执行的方式，则需要修改task的生成方式，不是直接使用异步函数，而是
使用\codeinline{rust}{tokio::spawn}生成，然后再通过\codeinline{rust}{join!}进行合并：
\begin{code-block}{rust}
...
#[tokio::main]
async fn main() {
    let task1 = tokio::spawn(hello());
    let task2 = tokio::spawn(call());
    tokio::join!(task1, task2);
}
\end{code-block}

上述代码是使用\codeinline{rust}{#[tokio::main]}这种注解方式实现的，如果不是使用
注解，而是直接使用tokio的运行时方式实现，则需要重点注意，比如下面的代码，运行就
会出现错误：
\begin{code-block}{rust}
macro_rules! __serial_command__ {
    ($location: expr) => {{
...
fn main() {
    ...
    let task1 = tokio::spawn(hello());
    let task2 = tokio::spawn(call());
    let (_, _) = rt.block_on(async move { tokio::join!(task2, task1) });
}
\end{code-block}

上述代码编译并没有问题，但是在运行的时候，就会出现下列错误：
\begin{code-block}{text}
thread 'main' panicked at 'there is no reactor running, must be called from the context of a Tokio 1.x runtime', testren/src/main.rs:77:17
note: run with `RUST_BACKTRACE=1` environment variable to display a backtrace
\end{code-block}
出现上述原因的问题，在于task1和task2是在tokio全局创建的任务，并没有和任何一个运行时
进行任何的绑定，使用tokio的注解执行时，tokio的运行时会自动创建绑定关系。但是当使用
\codeinline{rust}{join!}进行合并并执行时，tokio找不到一个与之对应的运行时实例来执行，
因此就会出错。而解决的思路很简单：将task与运行时实例进行绑定即可：
\begin{code-block}{rust}
macro_rules! __serial_command__ {
    ($location: expr) => {{
        println!("Sucess poweroff the {} board", $location);
...
fn main() {
    let rt = tokio::runtime::Runtime::new().unwrap();
    // 使用当前的runtime实例创建任务
    let task1 = rt.spawn(hello());
    let task2 = rt.spawn(async { __serial_command__!(123) });
    let (_, _) = rt.block_on(async move { tokio::join!(task2, task1) });
}
\end{code-block}
如此，上述代码即可正常运行。
...
\codeinline{rust}{select!}是Tokio当中非常重要的一个宏，它可以并行/并发的运行多个异步任务，
但是，只要其中一个成功返回了，就结束其他所有的异步任务。常见的使用规则如下：
\begin{code-block}{rust}
tokio::select! {
...
默认情况下，\codeinline{rust}{select!}会尝试匹配代码块当中的各个分支，一旦匹配到了，
则中断后续的匹配操作，并且取消其他分支当中的异步任务。如果没有匹配到，则执行\codeinline{rust}{else}
分支当中的代码。这个\codeinline{rust}{else}分支是可选择的，但是，如果没有这个分支，
当其他所有分支都没有匹配到时，则\codeinline{rust}{select!}宏会直接panic。

和之前的\codeinline{rust}{join!}类似，\codeinline{rust}{select!}的所有异步任务
全部是在同一个工作线程上\colorblock{并发}运行的，因此，如果有一个分支被阻塞，则其他分支都会无法继续
执行。如果需要所有分支都是\colorblock{并行}运行，则应当使用\codeinline{rust}{tokio::spawn}
生成异步任务，然后在\codeinline{rust}{select!}的分支当中使用这些任务。

通常，\codeinline{rust}{select!}会随机选择分支进行检查，这是满足大多数场景需求的模式。
如果必须要求从上至下的顺序进行执行，则需要在分支当中使用\codeinline{rust}{biased;}操作。
当然，由于一些其他的原因，我们可能会倾向于使用\codeinline{rust}{biased;}：
\begin{itemize}
    \item \codeinline{rust}{select!}的随机数（随机性）并不是0开销（存在cpu消耗）。
    \item 分支当中的future之间可能需要以轮询的顺序进行交互。
\end{itemize}

不过，使用这种顺序必须注意确保对future的轮训是完全公平的。比如，如果\codeinline{rust}{select!}
...
当使用\codeinline{rust}{select!}时，应当确保每个分支或者说每个任务是可以被安全的取消，
以避免丢失数据。注意，tokio的\codeinline{rust}{Mutex}、\codeinline{rust}{RwLock}、
\codeinline{rust}{Semaphore}以及\codeinline{rust}{Notify}都不是可以被安全取消的，
这些方法或者结构体要避免在\codeinline{rust}{select!}当中使用。
...
不仅针对异步方法/函数使用\codeinline{rust}{select!}，针对tokio的stream也可以使用：
\begin{code-block}{rust}
...
如果想完全按照分支的顺序进行执行，则需要使用上面提到的\codeinline{rust}{biased;}：
\begin{code-block}{rust}
#[tokio::main]
async fn main() {
    let mut count = 0u8;
...
但是，并不是所有的判断都可以在执行\codeinline{rust}{select!}之前进行，如果使用
...
\end{code-block}
在上面的例子中，\codeinline{rust}{sleep.is_elapsed()}可能返回true，即使\codeinline{rust}{sleep.poll()}
从未返回Ready，即未运行。这会产生一个潜在的竞争条件：\codeinline{rust}{while !sleep.is_elapsed()}进行
过期检查，以及根据\codeinline{rust}{sleep}是否过期而进行的\codeinline{rust}{select!}操作，
这会导致\codeinline{rust}{some_async_work()}被不间断的调用，即使\codeinline{rust}{sleep}已经结束。
...
\subsubsection{pin!}
在上述的代码当中，又出现了一个比较关键的宏：\codeinline{rust}{pin!}。所谓的pin，
实际上就是在堆栈当中将对应的变量固定住。默认情况下，对异步函数的调用会得到一个
被标记为\codeinline{rust}{!Unpin}的Future所包裹的值。如同之前的内容所述，在对这些
Future进行轮询操作（poll）之前，这些值必须被固定住。通常情况下，使用\codeinline{rust}{.await}
操作就可以解决这个问题，不过，该操作会消费对应的future。如果需要在\codeinline{rust}{&mut _}
上执行\codeinline{rust}{.await}操作，就必须要求调用者自行对future进行固定。固定
future可以只用\codeinline{rust}{Box::pin}或者使用\codeinline{rust}{pin!}这个宏。
...
此时，就必须使用\codeinline{rust}{pin!}进行修改：
\begin{code-block}{rust}
use tokio::pin;
...
在\codeinline{rust}{select!}和\codeinline{rust}{T: Stream + Unpin}这种stream的操作
当中，\codeinline{rust}{pin!}使用非常普遍。不过，在使用时，需要注意几点：
\codeinline{rust}{pin!}接收变量或者标识符作为参数，但是，\colorblock{无法接收表达式作为参数}，
因此，下面的代码是无法通过编译的：
\begin{code-block}{rust}
async fn my_async_fn() {
    // async logic here
...
和\codeinline{rust}{select!}配合使用时，则需要注意，被\codeinline{rust}{pin!}的
future无需再进行\codeinline{rust}{.await}操作：
\begin{code-block}{rust}
...
除此之外，Tokio也同样提供了异步的linux命令支持，并且和标准库（\codeinline{rust}{std::process}）
