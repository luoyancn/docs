...
\end{code-in-enumerate}
  ...
  \1 准备对应的二进制文件
需要的二进制文件主要包含了网络（calico:https://docs.projectcalico.org/
v3.1/releases/），安全（cfssl），kubernetes以及其他插件。
...
for id in {node,cni,kube-controllers};do docker pull quay.io/calico/calico-$id:v3.1.0;done
for id in {node,cni,kube-controllers};do docker pull reg.qiniu.com/quay/calico-$id:v3.1.1;done
...
for id in {kube-dns,dnsmasq-nanny,sidecar};do docker pull gcr.io/google-containers/k8s-dns-$id-amd64:1.14.9;done
...
\begin{code-in-enumerate}{bash}
sed -i -r \
    -e "s|__KUBE_DNS_DOMAIN__|k8s.zhangjl.m|" \
...
需要注意，ingress-controller已经不再放到gcr上了，同步的更新地址为https://github.com/kubernetes/ingress-nginx/releases。
\begin{code-in-enumerate}{bash}
sed -i -r \
