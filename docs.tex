...
\definecolor{codecolor}{RGB}{251,241,199}
...
\newcommand{\coloref}[1]{\hypersetup{linkcolor=RedOrange}\ref{#1}\hypersetup{linkcolor=black}}
...
% 改造code-block的实现，使其支持可选参数，可以根据需要添加minted的各项参数
% 比如增加逃逸代码等，需要注意，escapeinside表示逃逸代码的标记符成对出现，
% mathescape表示数学公式也启用逃逸。但是，escapeinside与mathescape会影响
% 到$符号的正常显示与输出，并且会提示错误，因此，如果需要使用$符号，应当如下：
% \begin{code-block}[escapeinside=~~,mathescape]{python}
% macro_rules! create_function {
%     // ~\mbox{\textdollar}~ident
%     (~\mbox{\textdollar}~func_name: ident) => {
%         fn ~\mbox{\textdollar}~func_name() {
%             info!("You called the {}()", stringify!(~\mbox{\textdollar}~func_name));
%         }
%     };
% }
% \end{code-block}
%
% 如果要创建minted代码当中指定行的引用，并能够跳转，则需要如下使用：
%
% \begin{code-block}[escapeinside=~~,mathescape]{python}
% def f(x):
%     """Square the given values.
%        x is an integer
%        returns x squared.
%     """
%     关键点就在那个\phantomsection指令上，否则会出现跳转有误的情况
%     y = ~\colorbox{green}{x**2}\phantomsection\label{m:pow}~
%     return y
% \end{code-block}
% minted的代码引用目前只能是根据行进行展示，只能使用\ref，不能使用\nameref
% see the line of \ref{m:pow}
% 如果定义了上述的namedlabel命令，则可以如下使用：
% \begin{code-block}[escapeinside=~~,mathescape]{python}
% def f(x):
%     """Square the given values.
%        x is an integer
%        returns x squared.
%     """
%     y = ~\colorbox{green}{x**2}\namedlabel{code}{\ding{202}}~
%     return y
% \end{code-block}
% minted的代码引用目前只能是根据行进行展示，只能使用\ref，但可以使用上面namelabel
% 所标记的名称
% see the line of \ref{code}
\newtcblisting{code-block}[2][]
%{beamer,colback=yellow!8!white,colframe=black,listing only,breakable, width=\linewidth,
{beamer, colback=gray!20!white,colframe=gray!35!white,listing only,breakable,width=\linewidth,
 ...
 breakanywhere,breaksymbolleft=,breakanywheresymbolpre=,#1}}

% 从文件中读取代码的实现
% 使用方式：\codefile{rust}{new.rs}
%\newtcbinputlisting{codefile}[2]
%{beamer, colback=gray!20!white, colframe=gray!35!white,
% listing only,breakable,width=\linewidth,
...
%    % 在tcolorbox当中显示行号，也可用于上面定义的code-block
%    %numbersep=2mm,linenos,},
%    %overlay={
...
\AddToShipoutPictureFG{%
  \put(\LenToUnit{-0.08cm},-3.3){
    \begin{tikzpicture}
    \node [opacity=0.5]{
      \ifodd\value{page}
        \includegraphics[scale=0.108]{yui.png}
      \fi
    };
    \end{tikzpicture}
  }
...
\AddToShipoutPictureFG{%
  \put(\LenToUnit{0.6cm},0){
    ...
    \node [opacity=0.5]{
      \ifodd\value{page}
      \else
        \includegraphics[scale=0.35]{nero.png}
      \fi
    };
    \end{tikzpicture}
  }
...
\AddToShipoutPictureFG{
  \put(\LenToUnit{\dimexpr \paperwidth-6.5cm},0)
  {
    \begin{tikzpicture}
    \node [opacity=0.5]{
      \ifodd\value{page}
        \includegraphics[scale=0.64]{bb.png}
      \fi
    };
    \end{tikzpicture}
  }
...
\AddToShipoutPictureFG{
  \put(\LenToUnit{\dimexpr \paperwidth-5cm},0)
  {
    ...
    \node [opacity=0.5]{
      \ifodd\value{page}
      \else
        \includegraphics[scale=0.08]{scathach.png}
      \fi
    };
    \end{tikzpicture}
  }
